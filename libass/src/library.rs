...
pub struct Library(*mut libass_sys::ass_library);
...
impl Library {
    ...
    pub fn set_style_overrides(&self, list: &[&str]) {
    ...
    pub fn new_renderer(&self) -> Option<Renderer> {
        let renderer = unsafe { libass_sys::ass_renderer_init(self.0) };
        if renderer.is_null() {
            return None;
        }
        Some(Renderer::new(renderer))
    }

    pub fn new_track(&self) -> Option<Track> {
        let track = unsafe { libass_sys::ass_new_track(self.0) };
        if track.is_null() {
            return None;
        }
        Some(Track::new(track))
    }
